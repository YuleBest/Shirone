...
- Select the narrowest relevant fragment, then add `tests/site/a11y.spec.ts` for page/component changes. Use `tests/site/icons.spec.ts` or `motion.spec.ts` when those contracts are touched.
